...
const teamHasEnoughPlayers = (room, team) => {
    const connectedOnTeam = room.players.filter((player) => player.team === team && player.connected).length;
    ...
    socket.on('start_round', async (payload, ack) => {
        ...
        try {
            ...
            if (!teamHasEnoughPlayers(room, room.game.nextTeam)) {
                throw new services_1.RoomServiceError('TEAM_NOT_READY', 'Active team needs at least 2 connected players to start a round', 409);
            }
